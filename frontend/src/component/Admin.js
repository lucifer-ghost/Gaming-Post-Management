...
const Admin = () => {
  ...
  useEffect(()=>{
    ...
    if(x==undefined){
      ...
      x=JSON.parse(x)
      axios.get("http://localhost:6969/getrposts",{"headers":{"Authorization":x.token}}).then((res)=>{
        setData(res.data)
      })
    }
  },[save])
  ...
  let accepting=(pid)=>{
    ...
    axios.get(`http://localhost:6969/accept/${pid}`,{"headers":{"Authorization":token}}).then(()=>{
      setSave(!save)
    })
  }
  ...
  let addComment=(pid)=>{
    let x=Cookies.get("logincrd")
    let {token}=JSON.parse(x)
    axios.put("http://localhost:6969/updrv",{"_id":pid,"msg":comment},{"headers":{"Authorization":token}}).then(()=>{
      setSave(!save) 
    })
  }
  return (
